...
async def get_status() -> Dict:
    ...
    try:
        async with httpx.AsyncClient(timeout=5, verify=False) as client:
            r = await client.get(f"{settings.CORTEX_URL}/api/user/current", headers=_headers(json=False))
...
async def list_analyzers() -> List[Dict]:
    ...
    try:
        async with httpx.AsyncClient(timeout=10, verify=False) as client:
            r = await client.get(f"{settings.CORTEX_URL}/api/analyzer", headers=_headers(json=False))
            if r.status_code == 200:
                return r.json()
    except Exception as e:
...
async def get_job(job_id: str) -> Optional[Dict]:
    ...
    try:
        async with httpx.AsyncClient(timeout=10, verify=False) as client:
            r = await client.get(f"{settings.CORTEX_URL}/api/job/{job_id}", headers=_headers(json=False))
            if r.status_code == 200:
                return r.json()
    except Exception as e:
...
async def get_job_report(job_id: str) -> Optional[Dict]:
    ...
    try:
        async with httpx.AsyncClient(timeout=10, verify=False) as client:
            r = await client.get(f"{settings.CORTEX_URL}/api/job/{job_id}/report", headers=_headers(json=False))
            if r.status_code == 200:
                return r.json()
    except Exception as e:
